...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'create') {
        ...
        if ($name === '' || $article === '') {
            $flash_error = 'Заполните название и артикул.';
        } elseif ($price < 0) {
            $flash_error = 'Цена не может быть отрицательной.';
        } else {
            $r = $admin_controller->createPart($name, $article, $price);
            $flash_error   = ($r['success'] ?? false) ? null : ($r['message'] ?? 'Ошибка.');
            $flash_success = ($r['success'] ?? false) ? 'Запчасть добавлена.' : null;
        }
        ...
        $article  = trim($_POST['article']    ?? '');
        $price    = (float) str_replace(',', '.', $_POST['price'] ?? '0');
        if ($id <= 0 || $name === '' || $article === '') {
            $flash_error = 'Некорректные данные.';
        } elseif ($price < 0) {
            $flash_error = 'Цена не может быть отрицательной.';
        } else {
            $r = $admin_controller->updatePart($id, $name, $article, $price);
            $flash_error   = ($r['success'] ?? false) ? null : ($r['message'] ?? 'Ошибка.');
            $flash_success = ($r['success'] ?? false) ? 'Запчасть обновлена.' : null;
        }
        ...
        if ($id <= 0) {
            ...
            $r = $admin_controller->deletePart($id);
            $flash_error   = ($r['success'] ?? false) ? null : ($r['message'] ?? 'Ошибка.');
            $flash_success = ($r['success'] ?? false) ? 'Запчасть удалена.' : null;
        }
    }
}

$parts = [];
$r = $admin_controller->getParts();
if ($r['success'] ?? false) {
    $parts = $r['data']['parts'] ?? [];
}
?>
...
<body>
    ...
    <div class="page">
        ...
        <h1 class="sans">Запчасти</h1>
        <p class="lead">Справочник запчастей склада. Используется при оформлении запросов на закупку.</p>
            ...
            <form method="post" action="">
                ...
                <div class="row2">
                    <div class="field">
                        ...
                        <input id="name" name="name" type="text" required maxlength="150" placeholder="Фильтр масляный">
                    </div>
                    <div class="field">
                        <label for="article">Артикул <span style="color:red;">*</span></label>
                        <input id="article" name="article" type="text" required maxlength="80" placeholder="OC90">
                    </div>
                </div>
                <div class="row2">
                    <div class="field">
        ...
        <section class="card">
            <h2>Список запчастей (<?php echo count($parts); ?>)</h2>
            <?php if (empty($parts)): ?>
                <p class="empty">Запчастей пока нет.</p>
            <?php else: ?>
                        ...
                        <tbody>
                            <?php foreach ($parts as $part): ?>
                                ...
                                <tr id="row-<?php echo $pid; ?>">
                                    ...
                                    <td><code><?php echo htmlspecialchars($part['article']); ?></code></td>
                                    <td><?php echo number_format((float) $part['price'], 2, '.', ' '); ?></td>
                                    <td>
                                        <button type="button" class="btn-sm"
                                            onclick="toggleEdit(<?php echo $pid; ?>)">Изменить</button>
                                        <form method="post" action="" class="inline-form">
                                            <input type="hidden" name="action" value="delete">
                                            <input type="hidden" name="id" value="<?php echo $pid; ?>">
                                            <button type="submit" class="btn-sm btn-sm-danger"
                                                onclick="return confirm('Удалить запчасть «<?php echo htmlspecialchars(addslashes($part['name'])); ?>»?')">Удалить</button>
                                        </form>
                                    </td>
                                </tr>
                                <tr class="edit-row" id="edit-<?php echo $pid; ?>">
                                    <td colspan="6">
                                        <form method="post" action="" style="display:flex;gap:10px;align-items:flex-end;flex-wrap:wrap;">
                                            ...
                                            <div class="field" style="margin:0;">
                                                <label>Артикул</label>
                                                <input type="text" name="article" class="w-art" required maxlength="80"
                                                    value="<?php echo htmlspecialchars($part['article']); ?>">
                                            </div>
                                            <div class="field" style="margin:0;">
                                                <label>Цена, ₽</label>
                                                <input type="number" name="price" class="w-price" required min="0" step="0.01"
                                                    value="<?php echo (float) $part['price']; ?>">
                                            </div>
                                            ...
                                            <button type="button" class="btn-sm" style="margin-bottom:0;"
                                                onclick="toggleEdit(<?php echo $pid; ?>)">Отмена</button>
                                        </form>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
